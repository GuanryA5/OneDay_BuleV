...
def fix_specific_issues(file_path: Path, content: str) -> str:
    ...
    # 2. 修复未使用的循环变量 (B007)
    content = re.sub(
        r"for category, node_types in", r"for _category, node_types in", content
    )

    # 3. 修复未使用的变量 (F841)
    content = re.sub(
        r"converted_value = self\.data_type\(value\)", r"self.data_type(value)", content
    )
    ...
    # 6. 修复异常链 (B904)
    content = re.sub(r"raise (\w+Error)\(([^)]+)\)", r"raise \1(\2) from e", content)
