...
class TestHandleException:
    ...
    def test_handle_bluev_exception(self):
        """测试处理BlueV异常"""
...
class TestExceptionContext:
    ...
    def test_exception_context_success(self):
        """测试异常上下文成功"""
    ...
    def test_exception_context_failure_reraise(self):
        """测试异常上下文失败并重新抛出"""
        with pytest.raises(BlueVValidationError):
            with exception_context("测试操作", reraise=True):
                raise ValueError("测试错误")

    def test_exception_context_failure_no_reraise(self):
        """测试异常上下文失败不重新抛出"""
